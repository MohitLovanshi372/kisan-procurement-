...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  window.addEventListener("languageChanged", () => {
    if (cachedProcurementData) {
      renderProcurementDetails(cachedProcurementData);
    }
  });
});
...
async function loadProcurementDetails() {
  ...
  try {
    ...
    if (!res.success || !res.data) {
      showToast(t("unableLoadProcDetails"), "error");
      return;
    }

    cachedProcurementData = res.data;
    renderProcurementDetails(cachedProcurementData);
    ...
    if (loading) loading.textContent = t("unableLoadProcInfo");
  }
}

function renderProcurementDetails(data) {
  const { procurement, centre } = data;
  const user = getUser();
  ...
  if (tokStatus) {
    tokStatus.textContent = t("activeToken");
  ...
  if (psStatus) {
    psStatus.innerHTML = `<span class="badge ${getStatusBadge(procStatus)}">${escapeHtml(procStatus)}</span>`;
  }
  ...
  if (psProgressText) psProgressText.textContent = `${percent}% ${t("completed")}`;
  ...
  if (payStatus) {
    payStatus.innerHTML = `<span class="badge ${getPaymentBadge(currentPayStatus)}">${escapeHtml(currentPayStatus)}</span>`;
  }
  ...
  if (currentPayStatus === "Paid") {
    ...
    if (payDate) payDate.textContent = t("notAvailableAwaiting");
    if (payTxId) payTxId.textContent = t("willBeGeneratedUponBank");
  }
}
